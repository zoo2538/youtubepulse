...
https.get('https://api.youthbepulse.com/api/unclassified?date=2025-10-26', (res) => {
  ...
  res.on('end', () => {
    try {
      ...
      // API 응답 구조 확인: {success: true, data: [...]} 또는 [...]
      const dataArray = result.data || result;
      
      console.log('═══════════════════════════════════════');
      console.log(`📊 수집된 데이터: ${dataArray.length}개`);
      console.log('═══════════════════════════════════════\n');
      
      if (dataArray.length === 0) {
        console.log('⚠️  오늘 수집된 데이터가 없습니다.');
        console.log('\n가능한 원인:');
        console.log('1. 크론잡이 아직 실행되지 않음 (09:00 KST 예정)');
        ...
        dataArray.slice(0, 5).forEach((item, index) => {
          ...
          console.log(`   제목: ${(item.videoTitle || item.video_title || 'N/A').substring(0, 50)}`);
