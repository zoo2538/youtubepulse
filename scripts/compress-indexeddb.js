...
console.log('🗜️ IndexedDB 압축/청소 시작...');

// 브라우저에서 실행할 IndexedDB 압축 스크립트
const browserCompressScript = `
// 브라우저 콘솔에서 실행할 IndexedDB 압축 스크립트
(async function compressIndexedDB() {
  try {
    console.log('🗜️ IndexedDB 압축/청소 시작...');
...
// 브라우저 스크립트를 파일로 저장
const browserScriptFile = path.join(EXPORT_DIR, 'browser-compress-script.js');
...
function simulateCompression() {
  ...
  // 시뮬레이션 결과 저장
  const simulationFile = path.join(EXPORT_DIR, 'compression_simulation.json');
...
// 메인 실행
async function main() {
  console.log('🚀 IndexedDB 압축/청소 실행...');
