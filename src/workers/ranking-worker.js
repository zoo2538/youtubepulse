...
// 공식 오피셜 채널 감지 함수
const isOfficialChannel = (channelName) => {
...
self.onmessage = function(e) {
  ...
  try {
    ...
    const todayChannelGroups = {};
    const videoIdSets = {}; // 고유 비디오 ID 추적
    ...
    // 랭킹 데이터 생성
    const rankings = Object.values(todayChannelGroups)
      .map((channel) => {
    ...
    self.postMessage({
      success: false,
      error: error.message || '랭킹 계산 중 오류가 발생했습니다.',
      stack: error.stack
    });
  }
};
