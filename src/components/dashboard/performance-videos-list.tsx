...
import { Star, TrendingUp, ArrowUpRight, Sparkles, CheckCircle2, Loader2, Key, Copy, Check, Eye } from "lucide-react";
...
export function PerformanceVideosList() {
    ...
    const loadPerformanceVideosData = async () => {
      try {
        ...
        if (allData && allData.length > 0) {
          ...
          // 성과 비율 계산 및 정렬
          // 먼저 videoId 기준으로 중복 제거 (같은 비디오가 여러 날짜에 수집된 경우 최신 조회수 사용)
  ...
  // AI 분석 함수
  const handleAnalyze = async (video: PerformanceVideoData) => {
  ...
  const handleCopyInsight = async (videoId: string) => {
    ...
    const video = performanceData.find(v => v.id === videoId);
              ...
              <div className="flex items-center justify-between">
                <div className="flex-1">
                  ...
                  <DialogDescription className="text-gray-600 dark:text-gray-400">
                    {performanceData.find(v => v.id === openDialogVideoId)?.title}
                  </DialogDescription>
                </div>
                <Button
                  variant="outline"
