    ...
    const loadDynamicCategories = async () => {
      try {
        ...
        if (savedCategories && Object.keys(savedCategories).length > 0) {
          console.log('📊 PerformanceVideosList - 저장된 카테고리 로드:', savedCategories);
          setDynamicSubCategories(savedCategories);
        } else {
          console.log('📊 PerformanceVideosList - 저장된 카테고리가 없음, 기본 카테고리 사용');
        }
      } catch (error) {
        console.error('PerformanceVideosList - 카테고리 로드 실패:', error);
      }
    };
